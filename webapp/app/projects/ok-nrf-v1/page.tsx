...
import { ArrowRight, ArrowLeft, Cpu, Radio, HardDrive, Lightbulb, Wifi, Shield } from "lucide-react";
import { ModelBackground } from "@/components/model-background";

const SPECS = [
  { icon: Cpu, label: "MCU", value: "ESP32-S3", detail: "Dual-core 240 MHz, Wi-Fi + BLE, 16 MB Flash" },
  { icon: Radio, label: "Radios", value: "3× nRF24L01+", detail: "2.4 GHz ISM, SPI bus, individual CS + IRQ" },
  { icon: Wifi, label: "Antenna", value: "SMA + U.FL", detail: "External 2.4 GHz SMA connectors per module" },
  { icon: HardDrive, label: "Storage", value: "microSD", detail: "SPI mode, push-push slot" },
  { icon: Lightbulb, label: "LEDs", value: "WS2812B + 4 RGB", detail: "NeoPixel + red/green/blue/yellow status LEDs" },
  { icon: Shield, label: "Interface", value: "USB-C + JST", detail: "USB-C 16P, JST-PH 3P/4P system integration" },
];

export default function OverkillNrfLandingPage() {
  return (
    <>
      {/* ── Fixed 3D background ── */}
      <ModelBackground modelPath="/Website/models/overkill-nrf.glb" color="#6366f1" />
      ...
      <section className="flex min-h-dvh flex-col items-center justify-center px-4 py-20 text-center">
        ...
        <p className="mt-4 max-w-lg text-[clamp(1rem,2vw,1.25rem)] text-muted-foreground animate-in fade-in slide-in-from-bottom-4 duration-700 delay-400">
          Modular RF communication board with 3× nRF24L01+ radio modules, microSD storage, and full Overkill system integration.
        </p>
        ...
        <div className="mt-8 flex flex-wrap justify-center gap-3 animate-in fade-in duration-700 delay-600">
          ...
          <Button render={<Link href="/projects/ok-nrf-v1/docs/" />} variant="outline" nativeButton={false}>
            Read the Docs
          </Button>
        </div>
      ...
      <section className="mx-auto w-full max-w-3xl px-4 py-20">
        ...
        <div className="space-y-4 text-muted-foreground">
          ...
          <p>
            ...
            An ESP32-S3-WROOM-1U-N16R8 handles processing, with external antenna support via SMA connectors.
            A microSD slot, 10 tactile switches (d-pad + center, reset, boot), and a WS2812B NeoPixel
            round out the interface. Power comes from USB-C with a TLV75733 3.3V LDO regulator.
          </p>
        </div>
      </section>
      ...
      <section className="mx-auto w-full max-w-3xl border-t border-border/40 px-4 py-20">
        <div className="flex flex-col items-center gap-6 text-center">
          ...
          <p className="max-w-md text-muted-foreground">
            Full schematics, BOM with LCSC part numbers, fabrication files, and firmware source.
          </p>
          <div className="flex gap-3">
            <Button render={<Link href="/projects/ok-nrf-v1/docs/" />} nativeButton={false}>
              Read the Docs
              <ArrowRight className="ml-1.5 h-4 w-4" />
            </Button>
